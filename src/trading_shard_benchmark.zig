...
const CanonicalInput = engine.CanonicalEvent;
...
const ShardInbox = struct {
    ...
    events: [capacity]CanonicalInput = undefined,
    head: usize = 0,
    len: usize = 0,
    high_water: usize = 0,

    fn push(self: *ShardInbox, event: CanonicalInput) !void {
    ...
    fn pop(self: *ShardInbox) ?CanonicalInput {
...
fn drainOne(shard: *QualifiedShard) !void {
    ...
    const command = try applyLive(&shard.run.shard, &shard.run.decision_journal, input);
    if (command != null)
        return error.UnexpectedCommand;
}
