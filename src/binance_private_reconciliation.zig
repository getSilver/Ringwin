...
pub const Reconciler = struct {
    ...
    fn emit(self: *Reconciler, scope: canonical.CanonicalEventScope, instrument: ?canonical.InstrumentIdentity, asset: ?canonical.AssetIdentity, identity: u128, source_time: ?u64, times: raw.Times, evidence: raw.RawEvidenceRef, event: canonical.Payload) !void {
    ...
    fn record(self: *Reconciler, scope: canonical.CanonicalEventScope, instrument: ?canonical.InstrumentIdentity, asset: ?canonical.AssetIdentity, identity: u128, source_time: ?u64, times: raw.Times, evidence: raw.RawEvidenceRef, event: canonical.Payload) !canonical.EventRecord {
        const binding = self.binding orelse return error.StaleSession;
