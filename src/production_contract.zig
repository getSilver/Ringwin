...
pub const schema_registry_identity: u64 = 6;
...
test "production contract freezes Linux scope and schema" {
    try std.testing.expectEqual(@as(u16, 8), journal_schema_version);
    try std.testing.expectEqual(@as(u32, 8), state_schema_version);
    try std.testing.expectEqual(@as(u16, 7), previous_journal_schema_version);
    try std.testing.expectEqual(@as(u32, 7), previous_state_schema_version);
