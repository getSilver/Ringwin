...
| `bybit-spot-testnet-v1` | Bybit Testnet + 指定账户 + BTCUSDT spot | §1.1 Bybit private/instrument messages；§3.7 public orderbook | `src/bybit_market_feed.zig` 的官方 V5 fixture、RawIngress、序列与 MarketFeedAdapter contract tests；`src/bybit_venue_adapter.zig` 与 `src/bybit_private_reconciliation.zig` 的 canonical command、order/fill 去重、完整 bootstrap/absolute observed 和共享 Adapter contract tests | `official_confirmed`；尚无真实 Testnet 准入与清理证据，发送仍 `U:authority`。 |
| `bybit-linear-testnet-v1` | Bybit Testnet + 指定账户 + BTCUSDT linear | §1.1 Bybit private/instrument messages；§3.7 public orderbook/ticker | 同上，含线性 mark/index/funding、cross sequence、linear position 与 VenueReduceOnly；尚无真实 Testnet 准入与清理证据 | `official_confirmed`；发送仍 `U:authority`。 |
