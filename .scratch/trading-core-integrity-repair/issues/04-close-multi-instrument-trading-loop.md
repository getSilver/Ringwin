...
- [ ] 同一 TradingShard 在一个确定轨迹中接受规范 SPOT 与 linear Instrument 的 OrderIntent，并产生各自正确的 OrderCommand。
- [ ] SPOT 现金约束与 linear 逐仓保证金约束分别使用对应注册规则，不能跨 Instrument 读取或释放 reservation。
- [ ] 两个 Instrument 的部分成交、终态、费用、Realized/UnrealizedPnL 和账本分录均按所有权独立闭合。
- [ ] 向一个 Instrument 发送另一 Instrument 的 ExecutionReport、Fill 或规则版本时原子拒绝，另一侧状态不变。
- [ ] live、全量 replay 和 snapshot-tail replay 的双 Instrument CanonicalStateDigest 相同。
...
- Spot and linear economics carry explicit product semantics into the shared economics projection and remain separated by product layer.
- Existing bounded multi-instrument lifecycle and SPOT/SWAP tests pass under the shared shard.
- `zig test src/main.zig`: 178/178 passed.
