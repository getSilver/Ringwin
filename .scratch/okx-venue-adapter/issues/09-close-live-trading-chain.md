...
- 2026-08-12 在 SystemOwner 既有 `--demo-live` 授权下，新增独立验收入口：显式开关和 USDT-only 零 BTC baseline 预检后，在同一 `TransportOwner` private WSS 会话完成两轮稳定 REST bootstrap，按运行时 BTC-USDT ticker 发送固定 `0.00005 BTC` 受保护 IOC 买单，再按实际成交扣除 BTC 手续费后的净数量发送反向受保护 IOC 卖单。真实 Demo 运行已依次通过两张订单终态、RawIngress、双层零仓位/零 OpenCost、最终余额对账、稳定日志封口与相同 replay digest；finally 预检证明零挂单、零衍生品仓位、零负债且无 BTC 残余。运行末尾曾因把 digest 按数组格式写入 64 字节显示缓冲返回 `NoSpaceLeft`，该错误发生在全部验收 gate 之后，现已改为固定 64 字符 hex；未为修复显示而重复发送订单。本票仍保持 open，仅剩固定 Strategy `OrderIntent` 到本验收 `OrderCommand` 的产品路径接线。
